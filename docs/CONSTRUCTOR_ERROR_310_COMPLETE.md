# Полная диагностика React Error #310 - Конструктор

## 1. Полный код constructor-client.tsx
...
## 2. Вывод pnpm why react react-dom
...
### Шаг 4: Скопировать ошибку

Скопируйте **полный** вывод из консоли:
- Все логи `[constructor]`
- Полный stack trace ошибки (все строки `at ...`)
- Все предупреждения и ошибки

### Пример ожидаемого вывода:

```
[constructor] render { botId: "...", wizardEnabled: true, hasParams: true, renderId: "abc123" }
[constructor] useQuery state { isLoading: true, hasData: false, hasError: false, botId: "..." }
[constructor] useQuery fetch started { botId: "..." }
Error: Rendered more hooks than during the previous render
    at renderWithHooks (react-dom.development.js:16305:17)
    at updateFunctionComponent (react-dom.development.js:19588:20)
    at updateSimpleMemoComponent (react-dom.development.js:19470:20)
    ...
```

---

## 4. Анализ количества хуков

### Хуки в constructor-client.tsx (в порядке вызова):

1. `useParams()` - строка 13
2. `useRouter()` - строка 14
3. `useQueryClient()` - строка 16
4. `useRef()` - строка 17
5. `useState()` x5 - строки 27-31
6. `useQuery()` - строка 47
7. `useMutation()` - строка 55
8. `useEffect()` x2 - строки 83, 89
...
1. **Ранние returns (строки 106-137):**
   - `if (isLoading) return ...` - **ПОСЛЕ** всех хуков ✅
   - `if (error || !botData) return ...` - **ПОСЛЕ** всех хуков ✅
   - `if (!schema) return ...` - **ПОСЛЕ** всех хуков ✅
...
4. **Изменение количества хуков:**
   - ⚠️ `useQuery` может быть disabled через `enabled: !!botId`
   - ⚠️ Но `useQuery` всегда вызывается, независимо от `enabled`

### Возможная причина:

Если `botId` меняется между рендерами (undefined → string), это не должно влиять на количество хуков, так как `useQuery` всегда вызывается.

**Но:** Если компонент размонтируется и монтируется заново с другим `botId`, React может увидеть разное количество хуков из-за кеширования React Query.

---

## 5. Чек-лист для диагностики

- [ ] Запущен `pnpm dev`
- [ ] Открыт `http://localhost:3000`
- [ ] Включены Source Maps в DevTools
- [ ] Кликнут "Конструктор"
- [ ] Скопированы все логи `[constructor]`
- [ ] Скопирован полный stack trace (все строки `at ...`)
- [ ] Скопирован вывод `pnpm -C packages/owner-web why react react-dom`
- [ ] Проверены Network запросы (статусы, тела ответов)
