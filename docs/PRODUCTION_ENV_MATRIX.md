...
|--------|-----------------|------------------|-------------------|-------------|-------------|
| **core** | `core-production-xxxx.up.railway.app` | `DATABASE_URL`<br>`TELEGRAM_BOT_TOKEN`<br>`ENCRYPTION_KEY`<br>`JWT_SECRET` | `REDIS_URL`<br>`TELEGRAM_SECRET_TOKEN`<br>`OWNER_BOTLINK_SECRET`<br>`MINIAPP_URL` (рекомендуется)<br>`MINI_APP_URL` (legacy)<br>`API_URL`<br>`SENTRY_DSN` | `GET /health` | PostgreSQL<br>Redis (опционально)<br>Telegram Bot API |
