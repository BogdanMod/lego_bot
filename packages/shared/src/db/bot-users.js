...
exports.createOrUpdateBotUserWithClient = createOrUpdateBotUserWithClient;
exports.getBotUsersWithClient = getBotUsersWithClient;
exports.getBotUserStatsWithClient = getBotUserStatsWithClient;
exports.exportBotUsersToCSVWithClient = exportBotUsersToCSVWithClient;
...
async function createOrUpdateBotUserWithClient(client, botId, telegramUserId, data) {
    ...
    const result = await client.query(`INSERT INTO bot_users (
        bot_id,
        telegram_user_id,
        first_name,
        last_name,
...
async function getBotUsersWithClient(client, botId, userId, params) {
    ...
    const cursorTimestamp = formatCursorTimestamp(last?.first_interaction_at);
...
async function getBotUserStatsWithClient(client, botId, userId) {
    const result = await client.query(`SELECT
        COUNT(*)::text as total,
...
async function exportBotUsersToCSVWithClient(client, botId, userId) {
    const result = await client.query(`SELECT
        bu.id,
        bu.bot_id,
        bu.telegram_user_id::text as telegram_user_id,
      ...
      JOIN bots b ON b.id = bu.bot_id AND b.user_id = $2
      WHERE bu.bot_id = $1
      ORDER BY bu.first_interaction_at DESC, bu.id DESC`, [botId, userId]);
    const headers = [
        'id',
        'bot_id',
        'telegram_user_id',
...
//# sourceMappingURL=bot-users.js.map