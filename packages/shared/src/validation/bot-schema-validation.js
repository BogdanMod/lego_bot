...
exports.validateBotSchema = validateBotSchema;
const limits_js_1 = require("../constants/limits.js");
const ip_validation_js_1 = require("../utils/ip-validation.js");
...
const isDisallowedHost = (hostname) => {
    ...
    if ((0, ip_validation_js_1.isIP)(lower)) {
        return isIpPrivate(lower);
    }
    return false;
};
const isAllowedByAllowlist = (hostname, allowlist) => {
...
const isValidHttpsUrl = (value, maxLength) => {
...
function validateBotSchema(schema) {
    const errors = [];
    ...
    const states = schemaObj.states;
    const initialState = schemaObj.initialState;
    if (!states[initialState]) {
        errors.push('Initial state does not exist in states');
    }
    const stateKeys = Object.keys(states);
    if (stateKeys.length > limits_js_1.BOT_LIMITS.MAX_SCHEMA_STATES) {
        errors.push(`Schema exceeds maximum states of ${limits_js_1.BOT_LIMITS.MAX_SCHEMA_STATES}`);
    }
    ...
    for (const [stateKey, state] of Object.entries(states)) {
        ...
        else if (stateObj.message.length > limits_js_1.BOT_LIMITS.MAX_MESSAGE_LENGTH) {
            errors.push(`State "${stateKey}" message exceeds maximum length of ${limits_js_1.BOT_LIMITS.MAX_MESSAGE_LENGTH}`);
        }
        if (stateObj.parseMode !== undefined) {
            if (stateObj.parseMode !== 'HTML' &&
        ...
        if (stateObj.media !== undefined) {
            if (!stateObj.media || typeof stateObj.media !== 'object' || Array.isArray(stateObj.media)) {
                ...
                const media = stateObj.media;
                if (!media.type || typeof media.type !== 'string' || !limits_js_1.MEDIA_LIMITS.ALLOWED_MEDIA_TYPES.includes(media.type)) {
                    errors.push(`State "${stateKey}" media.type is invalid`);
                }
                if (!media.url || typeof media.url !== 'string') {
                    errors.push(`State "${stateKey}" media.url must be a string`);
                }
                else if (!isValidHttpsUrl(media.url, limits_js_1.MEDIA_LIMITS.MAX_MEDIA_URL_LENGTH)) {
                ...
                if (media.caption !== undefined) {
                    ...
                    else if (media.caption.length > limits_js_1.MEDIA_LIMITS.MAX_CAPTION_LENGTH) {
                        errors.push(`State "${stateKey}" media.caption exceeds maximum length of ${limits_js_1.MEDIA_LIMITS.MAX_CAPTION_LENGTH}`);
                    }
                }
                if (media.cover !== undefined) {
                    if (typeof media.cover !== 'string') {
                        ...
                        if (!isValidHttpsUrl(media.cover, limits_js_1.MEDIA_LIMITS.MAX_MEDIA_URL_LENGTH)) {
                ...
                if (media.thumbnail !== undefined) {
                    if (typeof media.thumbnail !== 'string') {
                        ...
                        errors.push(`State "${stateKey}" media.thumbnail requires multipart/form-data (attach://...) in v1; use cover (URL) instead`);
                    }
                }
            }
        }
        if (stateObj.mediaGroup !== undefined) {
            if (!Array.isArray(stateObj.mediaGroup)) {
                ...
                if (stateObj.mediaGroup.length < 2 || stateObj.mediaGroup.length > limits_js_1.MEDIA_LIMITS.MAX_MEDIA_GROUP_SIZE) {
                    errors.push(`State "${stateKey}" mediaGroup size must be between 2 and ${limits_js_1.MEDIA_LIMITS.MAX_MEDIA_GROUP_SIZE}`);
                }
                for (const item of stateObj.mediaGroup) {
                    ...
                    else if (!isValidHttpsUrl(mediaItem.url, limits_js_1.MEDIA_LIMITS.MAX_MEDIA_URL_LENGTH)) {
                    ...
                    if (mediaItem.caption !== undefined) {
                        ...
                        else if (mediaItem.caption.length > limits_js_1.MEDIA_LIMITS.MAX_CAPTION_LENGTH) {
                            errors.push(`State "${stateKey}" mediaGroup item caption exceeds maximum length of ${limits_js_1.MEDIA_LIMITS.MAX_CAPTION_LENGTH}`);
                        }
                    }
                }
            }
        ...
        if (stateObj.buttons !== undefined) {
            ...
            if (stateObj.buttons.length > limits_js_1.BOT_LIMITS.MAX_BUTTONS_PER_STATE) {
                errors.push(`State "${stateKey}" exceeds maximum buttons of ${limits_js_1.BOT_LIMITS.MAX_BUTTONS_PER_STATE}`);
            }
            ...
            for (const button of stateObj.buttons) {
                ...
                else if (buttonObj.text.length > limits_js_1.BOT_LIMITS.MAX_BUTTON_TEXT_LENGTH) {
                    errors.push(`Button text exceeds maximum length of ${limits_js_1.BOT_LIMITS.MAX_BUTTON_TEXT_LENGTH}`);
                }
                if (buttonType === 'url') {
                    if (!buttonObj.url || typeof buttonObj.url !== 'string') {
                        errors.push(`Button url in state "${stateKey}" must be a string`);
                    }
                    else if (!isValidHttpsUrl(buttonObj.url, limits_js_1.MEDIA_LIMITS.MAX_MEDIA_URL_LENGTH)) {
        ...
        if (stateObj.webhook !== undefined) {
            if (!stateObj.webhook || typeof stateObj.webhook !== 'object' || Array.isArray(stateObj.webhook)) {
                ...
                const webhook = stateObj.webhook;
                ...
                if (webhook.url !== undefined) {
                    if (typeof webhook.url !== 'string' || webhook.url.length === 0) {
                        ...
                        if (webhook.url.length > limits_js_1.WEBHOOK_INTEGRATION_LIMITS.MAX_URL_LENGTH) {
                ...
                if (webhook.headers !== undefined) {
                    if (!webhook.headers || typeof webhook.headers !== 'object' || Array.isArray(webhook.headers)) {
                        ...
                        if (entries.length > limits_js_1.WEBHOOK_INTEGRATION_LIMITS.MAX_HEADERS) {
                        ...
                        for (const [headerKey, headerValue] of entries) {
                            ...
                            if (headerValue.length > limits_js_1.WEBHOOK_INTEGRATION_LIMITS.MAX_HEADER_VALUE_LENGTH) {
                ...
                if (webhook.signingSecret !== undefined) {
                    ...
                    else if (webhook.signingSecret.length > limits_js_1.WEBHOOK_INTEGRATION_LIMITS.MAX_SIGNING_SECRET_LENGTH) {
                        errors.push(`State "${stateKey}" webhook.signingSecret too long`);
                    }
                }
                if (webhook.retryCount !== undefined) {
                    if (typeof webhook.retryCount !== 'number' ||
                        !Number.isInteger(webhook.retryCount) ||
                        webhook.retryCount < 0 ||
                        webhook.retryCount > limits_js_1.WEBHOOK_INTEGRATION_LIMITS.MAX_RETRY_COUNT) {
                        errors.push(`State "${stateKey}" webhook.retryCount must be a non-negative integer`);
                    }
                }
                if (webhook.timeout !== undefined) {
                    if (typeof webhook.timeout !== 'number' ||
                        webhook.timeout <= 0 ||
                        webhook.timeout > limits_js_1.WEBHOOK_INTEGRATION_LIMITS.TIMEOUT_MS) {
        ...
        if (stateObj.integration !== undefined) {
            if (!stateObj.integration || typeof stateObj.integration !== 'object' || Array.isArray(stateObj.integration)) {
                ...
                const integration = stateObj.integration;
    ...
    if (enabledWebhookCount > limits_js_1.WEBHOOK_INTEGRATION_LIMITS.MAX_WEBHOOKS_PER_BOT) {
        errors.push(`Schema exceeds maximum webhooks of ${limits_js_1.WEBHOOK_INTEGRATION_LIMITS.MAX_WEBHOOKS_PER_BOT}`);
    }
    return { valid: errors.length === 0, errors };
}
//# sourceMappingURL=bot-schema-validation.js.map