...
rem Safety net: only run npm ci if we're on CI AND node_modules is unexpectedly missing.
...
rem PATCH (CI only):
rem We copy built artifacts into core\node_modules so the runtime can resolve
...
rem
rem Ensure dist/bot-instance.js exists for serverless runtime resolution
rem (re-exports botInstance/botInitialized/ensureBotInitialized from ./index.js)
rem
if not exist dist mkdir dist
> dist\\bot-instance.js (
  echo // Generated by build.sh
  echo const core = require('./index.js'^);
  echo module.exports = {
  echo   botInstance: core.botInstance,
  echo   botInitialized: core.botInitialized,
  echo   ensureBotInitialized: core.ensureBotInitialized,
  echo };
)

rem
rem Ensure dist/init-bot.js exists for serverless runtime resolution
rem (re-exports initBot/ensureBotInitialized from ./index.js)
rem
> dist\\init-bot.js (
  echo // Generated by build.sh
  echo const core = require('./index.js'^);
  echo module.exports = {
  echo   initBot: core.initBot,
  echo   ensureBotInitialized: core.ensureBotInitialized,
  echo };
)
